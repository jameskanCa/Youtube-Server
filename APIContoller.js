...
function addNewSession(req, res) {
  userInfo.findOne({ userId: req.body.userId }, "userId session", function (
    err,
    user
  ) {
    if (err) {
    ...
    if (user == null) {
      ...
      let sessionCheck = user.session.find((sessionTemp) => {
        return (
          sessionTemp.videoId.toString() ==
          req.body.session[0].videoId.toString()
        );
      });
...
module.exports = { addNewSession, addEndSession, deleteSession };
